...
// Initialize key pair (will be generated in test setup)
export async function initializeKeyPair() {
  if(!rsaMultikeyKeyPair) {
    rsaMultikeyKeyPair = await RsaMultikey.generate({
      controller,
      modulusLength: 2048
    });
    const publicKey = await rsaMultikeyKeyPair.export({
      publicKey: true,
      includeContext: true
    });
    mockPublicRsaMultikey = publicKey;
...
// Export getters that will return the initialized values
export function getRsaMultikeyKeyPair() {
  return rsaMultikeyKeyPair;
}

export function getMockPublicRsaMultikey() {
  return mockPublicRsaMultikey;
}

export function getControllerDocRsaMultikey() {
  return controllerDocRsaMultikey;
}
